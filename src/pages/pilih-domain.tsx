...
import { FileMap, itAspek, itDomain, itIndikator } from '@/typeData/itIndikator'
...
const PilihDomain = () => {
  ...
  const [fileState, setFileState] = useState<Record<number, File>>({})
  ...
  const _handleJawabanChange = (id: number, newJawaban: string) => {
    setPertanyaan(prev =>
      prev.map(q => (q.id_kuisioner === id ? { ...q, jawaban: newJawaban } : q))
    )
  }
  ...
  const kirimJawaban = () => {
    ...
    for (const id in fileState) {
      formData.append(`file_upload_${id}`, fileState[id])
    }
    ...
    axiosCostume
      ...
      .then(res => {
        ...
        if (res.data.hasil > 85) {
          ...
          alert(`Karena nilai lebih dari 85, maka lanjut ke level ${level + 1}`)
        } else {
          if (selectIndikator) {
            setIndikatorTerjawab(c => [...c, selectIndikator])
          }
